...
def _plot_curves(
    ...
    for ds in df_best['dataset'].unique():
        ...
        unique_bs = sorted(b for b in df_noise_ds['BatchSize'].unique() if b != -1)
...
def main() -> None:
    ...
    for f in files:
        ...
        csv_paths = _discover_files('subset-data-csvs/*noise_multiplier*.csv')
        if not csv_paths:
        ...
        global_ymin = min(all_ymin)

        print(F'ymin: {global_ymin}, ymax: {global_ymax}')
